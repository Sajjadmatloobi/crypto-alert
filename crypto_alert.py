#!/usr/bin/env python3
"""
Crypto 100%+ Growth Alert Bot (All Coins - via CoinGecko)
===========================================================
این نسخه برای اجرا روی GitHub Actions طراحی شده: هر بار که اجرا میشه
یک‌بار چک می‌کنه و تموم میشه (خود GitHub Actions طبق زمان‌بندی صداش می‌زنه).

توکن‌ها از Environment Variables خونده میشن (نه اینکه تو کد نوشته بشن):
  TELEGRAM_BOT_TOKEN
  TELEGRAM_CHAT_ID
"""
...
def fetch_all_coins():
    ...
    for page in range(1, PAGES_TO_FETCH + 1):
        url = COINGECKO_URL.format(per_page=PER_PAGE, page=page)
        try:
            data = fetch_json(url)
